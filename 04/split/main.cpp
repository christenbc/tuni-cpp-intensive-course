...
std::vector<std::string> split(std::string line, char separator, bool ignore = false){
    ...
    if(!ignore){ //don't ignore, include empty parts
        for(std::string::size_type i = 0; i < line.length(); i++){ //size_type is compatible with line.size
            if(line[i] == separator){ //if the character matches with separator
            ...
            }else if(line[i] != separator){ //if the character is different than the separator
        ...
        for(std::string::size_type i = 0; i < line.length(); i++){
            if(i == 0 && line[i] == separator)//if in the first character there is a separator
                mark = true;//??
            else if(line[i] == separator && !mark){//if there is a separator but not in the first character
                mark = true;
                result.push_back(temp);
                temp = ""; //??
            }else if (line[i] != separator){ //if there is no separator on that character
                mark = false;
                temp += line[i];
                if(i == line.size()-1)
                    result.push_back(temp);
